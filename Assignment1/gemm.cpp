...
__m256 *Am = reinterpret_cast<__m256*>(A);
__m256 *Bm = reinterpret_cast<__m256*>(B);
__m256 *Cm = reinterpret_cast<__m256*>(C);
...
float Bf[N*N] __attribute__ ((aligned (64)));
    __m256 *Bfm = reinterpret_cast<__m256*>(Bf);
...
int main() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    for (int j = 0; j < NTHREADS; j++) {
      pthread_create(&threads[j], nullptr, matmul_thread, (void*)(int64_t)j);
    }
